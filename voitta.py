...
# from lib.voitta_canvas import CanvasDescription
...
class EndpointDescription:
    ...
    async def call_function(self, name, arguments, token, oauth_token):
        # print (f"---------- call_function({token}, {oauth_token})")
        ...
        if tool.method == "get":
            ...
            if tool.schema is None or len(tool.schema) == 0:
                ...
                # print (f"get with arguments, final url: {url}")
                async with httpx.AsyncClient() as client:
                    response = await client.get(url, headers=headers)
                    return response.text
...
"""
                ...
                print(f"DSPY function_name: {function_name}")
                print(function_desc, "\n")
                print(func_text, "\n")
    ...
    async def call_function(self, name, arguments, token, oauth_token, tool_call_id):
        ...
        # for argument in arguments:
        #     print (f"argument: {argument}, value:{arguments[argument]}")
...
# voittaRouter = VoittaRouter(["https://agnitio-assets-be.owlsdont.com"])
